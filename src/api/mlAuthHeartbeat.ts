/**
 * ML auth heartbeat — periodically verifies the cli-web-musicleague session is
 * still valid against the live server, and exposes the cached result.
 *
 * The CLI's `auth status` only inspects the local auth.json file; it doesn't
 * prove the cookies still work server-side. We probe a cheap authenticated
 * endpoint (users me) instead and react to AUTH_EXPIRED.
 */
import { spawn } from 'node:child_process';

export type MlAuthState = {
...
const HEARTBEAT_INTERVAL_MS = 5 * 60 * 1000; // 5 min — well under any reasonable token TTL
...
/** Force a recheck. De-duplicates concurrent calls. */
...
async function runProbe(): Promise<MlAuthState> {
  ...
  // Try to parse the JSON body either way (success or structured error)
  ...
  if (
    ...
    (payload as { error?: boolean }).error === true
  ) {
    ...
    cached.status = 'unknown';
    cached.message = (payload as { message?: string }).message ?? `Probe failed: ${code || result.exitCode}`;
    return { ...cached };
  }

  if (result.exitCode === 0 && payload && typeof payload === 'object' && 'id' in (payload as object)) {
    cached.status = 'ok';
...
export function startMlAuthHeartbeat(): void {
  if (timer) return;
  // Fire once at startup, then on a fixed interval.
