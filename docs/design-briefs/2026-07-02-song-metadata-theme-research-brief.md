...
- **⭐ Complete metadata catalog:** `docs/design-briefs/2026-07-02-song-metadata-catalog-for-cd.md` — the **authoritative, full** per-song field list (types, ranges, coverage, live-vs-planned status, and a "do NOT design for these" section). **Read this first for the data model** — it supersedes the shorter field lists in §3/§10 of this brief and includes fields we're about to add (release year + deeper lyrics metrics: sentiment, explicit, readability, vocabulary).
