...
4. **Archive cards show "—" for the winning song** (sprint-32 live eyeball) — the read-model `archive[]` entries' `winnerSong`/`winnerArtist`/submitter are sparse/null, so the Digest Archive renders a placeholder dash. Populate them in the read-model generator's archive slice.

## the b-side — content polish sprint (drafted 2026-06-15) — owner wants AFTER the campaign (sprint-34 candidate)
**Source:** owner, 2026-06-15, after walking the live b-side ("very happy, but there are some content things we need to tweak"). Hold a dedicated **content-polish pass** once the campaign (sprint-31/32/33) is complete: tune the generated read-model content + the site's presentation of it. Bundles the sprint-31 read-model nits above (fan/hater name+accent, empty playlist agenda, moment round labels, archive winnerSong) PLUS owner's content tweaks (to be enumerated — likely superlative/KPI/blurb wording, what each section emphasizes, the discovery-playlist voice). Sequence AFTER sprint-33 (operator Content screen) so the operator can re-publish/steer the polished content via the UI rather than orc curl. Owner will provide the specific content tweaks then.
...
**Sequencing:** HIGH priority but **after** the b-side campaign (sprint-31/32/33) and the Universal Share button sprint land — do the hygiene pass once that work is itself trackable, so we backfill everything at once rather than twice.
