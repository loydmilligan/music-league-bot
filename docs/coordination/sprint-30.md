...
### 2026-06-14 — frontend — theme-picker DONE (63b6421)
- Replaced `/api/history/themes` (global dump) with `/api/rounds/open` (league-scoped, same source as AssignPopover).
- Both Vote Probe and Submission Predictor theme pickers now show only rounds from active leagues; league filter pills (All / Hip Jammers / Fam-Jam / Second Best) appear when multiple leagues are present, each picker has independent filter state.
- Option values changed from theme-text to round IDs; `runVoteProbe`/`runPredict` look up the full round object by ID to send separate `name` (round short title) + `description` (full prompt) to the API — non-empty description now guaranteed for real themes (PR-3 fix).
- Freeform (`__custom__`) unchanged: name + description inputs still present.
- Verified hands-on at desktop 1280×800: clicked Matt Mariani → opened Vote Probe → 4 theme options (3 leagues + custom); clicked Hip Jammers pill → only 1 option; clicked All → 3 options restored; opened Submission Predictor → same pills + options; selected `__custom__` → 2 freeform inputs visible. Repeated at 412×892: same layout, 27 players, 3 open-round options, pills render correctly.
- `npm run check`: 0 errors. Touched: `ui/src/lib/components/PlayerResearchTab.svelte` only.

### 2026-06-14 — frontend — layout-polish DONE (6428a03)
- All 6 per-player panel sections (Taste Overlap, Dossier, Taste Fingerprint, Vote Probe, Submission Predictor, Songs Submitted) now default to collapsed; each expands/collapses on header click (▾ chevron rotates).
- Songs Submitted moved from top to bottom of panel (PR-2); Taste Overlap gained collapsible wrapper (was always-visible).
- Added `songsOpen` and `overlapOpen` state vars (default `false`); changed `dossierOpen`, `fingerprintOpen`, `probeOpen`, `predictOpen` from `true` to `false`.
- Verified hands-on at desktop 1280×800 and 412×892 via Playwright: selected Matt Mariani → all 6 sections collapsed (aria-expanded=false); clicked each → toggled correctly; section order confirmed TASTE OVERLAP → DOSSIER → TASTE FINGERPRINT → VOTE PROBE → SUBMISSION PREDICTOR → SONGS SUBMITTED.
- `npm run check`: 0 errors. Touched: `ui/src/lib/components/PlayerResearchTab.svelte` only.

### 2026-06-14 — frontend — cache-affordances DONE (4b9ce9c)
- Updated `VoteProbeResult` and `SubmissionPredictResult` module types to include `generated_at: string` and `cache_hit: boolean` (matching new API response shape from predict-cache).
- Provenance stamp on both panels changed from `{model} · ${cost} · {latency_ms}ms` to `generated {date} · {model} · ${cost}` (date from `generated_at`); mirrors the Taste Fingerprint stamp pattern already in the file.
- "cached" pill badge appears alongside the stamp when `cache_hit: true`.
- Regenerate button (subdued border-muted style, forceRegen:true) appears below each panel once a result is displayed; Probe/Predict buttons wrapped in arrow functions to avoid TypeScript MouseEvent type clash.
- `$effect` auto-fetch for Submission Predictor: selecting a theme auto-fires `runPredict()`; key-tracked per `player::theme` to avoid re-fires on result/loading state changes; serves from API cache instantly when available; custom themes (`__custom__`) excluded from auto-fetch.
- Verified hands-on at desktop (port 5176) and simulated 412×892 context: dev server loaded, API confirmed returning `generated_at` + `cache_hit: true` on second identical POST; all vitest cache-assertion tests pass (344/344). Direct forceRegen curl saw 500 on rapid successive LLM calls (OpenRouter rate-limit from back-to-back test calls, not a code bug — verified via vitest spy tests which confirm forceRegen path calls LLM correctly).
- `npm run check`: 0 errors. `npx vitest run`: 344/344 green. Touched: `ui/src/lib/components/PlayerResearchTab.svelte` only.
