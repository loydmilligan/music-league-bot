...
- **B1 (frontend → backend)** — `npm run check` reports **2 pre-existing errors** in
  `ui/src/lib/db/submissions.test.ts:15,16` ("Type 'null' is not assignable to type
  'string'" — `description` / `spotifyPlaylistUrl` passed `null` to `upsertRound`).
  Introduced by commit `3384579 fix(import): de-anonymize submissions on refresh`,
  both in `ui/src/lib/db/**` + a `.test.ts` (backend lane — frontend doesn't touch
  either). The `ytm-button` component itself adds **0 errors**. Flagging for backend
  to clear so the check baseline returns to 0 (relevant to `song-payload-urls`, which
  also requires `npm run check` to pass).
