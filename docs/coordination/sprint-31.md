...
- [x] {agent: backend, id: publish-api, depends: build-readmodel} **First-publish — slug mint + `publishSite` + endpoint** (spec §10). New `ui/src/lib/dashboard/publish.ts`: `publishSite(db, leagueId) → { slug }` — mint a permanent ≥80-bit slug (or reuse the league's existing one), run `buildReadModel` over all finalized rounds, persist `read_model` + `archived_rounds` + timestamps to `dashboard_sites`; idempotent re-publish refreshes in place on the same slug. Expose `POST /api/content/:leagueId/publish` (the operator UI in sprint-33 will call it; here it's directly callable/testable).
...
### 2026-06-14 — backend — build-readmodel complete (7dfd66b)
- `ui/src/lib/dashboard/buildReadModel.ts`: `buildReadModel(db, leagueId, opts?) → ReadModel` — composes all generator slices; fingerprints loaded from `player_profiles` (generate if missing via sprint-28 harness); full-tier members get spectrum + superlatives + fan/hater blurbs + voteTwins + voteTogether + playlist + biggestHater; lite-tier members get signatureSuperlative + biggestFan only (absent sections omitted, no empty blocks); league reel requires ≥2 full members; moments get LLM caption lines or null; archive built from historical rounds
- `ui/src/lib/dashboard/buildReadModel.test.ts`: 11 tests covering ReadModel schema validity, full+lite member correct fields, lite omits rewards/punishes/spectrum/biggestHater/superlatives/voteTwins/voteTogether/playlist, accent values in allowed set, stat fields numeric for all members, archive entries, reel shape, moments null when no data
- **Export added**: `ACCENT_VALUES` re-exported from buildReadModel.ts (test imports it); `void AccentSchema` removed (redundant after re-export)
- **Mock fix**: `setupMock()` dispatch order — `NO_STRIFE_CONSTRAINT` embeds "superlatives" in every prompt; changed first branch from `|| sys.includes('superlative')` to `sys.includes('yearbook writer')` only (unique to playerSuperlativesTask) so fan-hater branch resolves correctly
- 11/11 buildReadModel vitest green; 455/455 full suite green; 0 svelte-check errors
- `[-]` → `[x]`

### 2026-06-14 — backend — publish-api complete (1c199e5)
- `ui/src/lib/dashboard/publish.ts`: `publishSite(db, leagueId) → { slug }` — mints a ≥80-bit slug on first publish via `mintSlug()` (or reuses existing `dashboard_sites.slug`); runs `buildReadModel`; persists `read_model` + `archived_rounds` + timestamps to `dashboard_sites`; idempotent re-publish UPDATEs in place and bumps `refreshed_at`
- `ui/src/routes/api/content/[leagueId]/publish/+server.ts`: `POST /api/content/:leagueId/publish` — calls `publishSite`, returns `{ slug }`; bad leagueId → 400; non-existent league → 404
- `ui/src/lib/dashboard/publish.test.ts`: 4 tests — slug returned on first publish (≥80-bit check), valid `read_model` JSON persisted, second publish reuses slug + bumps `refreshed_at`, throws on bad leagueId
- 4/4 publish vitest green; 459/459 full suite green; 0 svelte-check errors
- `[-]` → `[x]`

### 2026-06-14 — backend — harness-json-robust complete (8fe8441)
- `ui/src/lib/predict/predict.ts`: added `extractJson(content): unknown | null` helper (exported) — strips `\`\`\`json`/`\`\`\`` code fences then extracts the first balanced `{...}` object, ignoring trailing prose
- Applied `extractJson` at both parse sites (initial parse line ~67, retry parse line ~94); bare JSON / clean JSON paths unaffected
- `ui/src/lib/predict/predict.test.ts`: +9 tests — `extractJson` unit tests (clean, bare, fenced, fenced+prose, non-JSON, empty) + `runPrediction` integration tests for fenced+prose initial response, fenced+prose retry response, and bare `{...}` response
- 468/468 full suite green; 0 svelte-check errors
- `[ ]` → `[x]`
