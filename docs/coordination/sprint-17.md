...
### 2026-06-03 — backend — discoverability coverage: backfilled ALL seasons + partial-coverage self-suppress (deployed)

Two follow-ups from the Fam Jam S3 UAT (lonely 1-row leaderboard). No rescale, no repeat-picks visual this pass (per brief).

- **Coverage backfill (all seasons).** Ran the fixed normalized-search backfill (`scripts/backfill-popularity.ts`, no-arg = all unfetched) for the 493 songs that were never fetched (only HJ S3's 27 were). **`song_popularity` now holds all 520 distinct submitted tracks; 492 with data, proxies recomputed over the full 520-song corpus.** Per-season coverage is now **100%** for every digested season: fam-jam S1 (82), S2 (100), S3 (116); hip-jammers S1 (77), S2 (97), S3 (27); second-best S1 (51).
  - **Prod verify — Fam Jam S3 r101:** discoverability now returns **all 11 players** (Em 26 most-obscure → arigrey83 13), where before only `arigrey83` showed (1 row, via a stray HJ uri overlap). ✅
  - **Corpus-relative note:** because `popularity_proxy` is normalized over the whole corpus, recomputing across 520 songs (vs the prior 27) nudged HJ S3's absolute obscurity values (e.g. Mashew 25→26, Ronm 12→13, Kristin 6→8) — but the **ranking order is unchanged** (Mashew #1 … Kristin #9). The clean per-song listeners/playcount the user reviewed are unchanged; only the relative 0-100 proxy shifted. (The scale rescale is still the open follow-up.)
