...
### 2026-06-05 — frontend — mobile-digest-menu DONE + prod-verified → **frontend lane CLOSED; SPRINT-19 COMPLETE** (commit 0d54124)

Added an **additive mobile/PWA nav affordance** below `md`; the desktop rail (`hidden md:flex`) is **untouched** (per D2). Fixes the bug where phone + installed-PWA users had **no nav at all** and couldn't reach digest generation.

**Implementation (`ui/src/routes/+layout.svelte`, frontend lane only):**
- Root layout `flex` → `flex flex-col md:flex-row` so a mobile top bar can stack above `<main>` on phones; on `md+` it's `flex-row` = the original desktop layout, unchanged.
- **`md:hidden` sticky top bar** (`z-30`): the `m/l` wordmark + a **hamburger (☰)** that opens a slide-in drawer.
- **Drawer** (`fixed`, `z-40` — deliberately **under** the `z-50` digest `.dg-modal-scrim` so a modal opened from a page still sits on top): light scrim + `fly`-in panel, Mash Co. header (wordmark + `MlAuthBadge`) + footer (`v{appVersion}`). Closes on **tap-outside / Esc / route change** (`$effect` tracking `page.url.pathname`).
- **No nav drift:** extracted the nav `<ul>` into a `{#snippet navList()}` rendered by **both** the desktop rail and the drawer — single source of truth.

**Coordination / deploy (serialized — no competing build):** when I went to deploy, backend's `bot-ui-on-base` was **mid-flight** — a `--no-cache bot-ui` build was running on the refactored Dockerfile.ui. Rather than collide (serialize-deploys rule), I **committed my change** (`0d54124`) and let backend's in-flight build carry it. My saved `+layout.svelte` was in the build context, so backend's **~59 s base-image build** shipped it to prod. **Confirmed live:** served `/` HTML contains `md:flex-row`, `mobile-nav-drawer`, `aria-label="Open navigation menu"`. (My own earlier `--no-cache bot-ui` attempt on the *old* slow Dockerfile.ui was the one killed mid-chromium-download — superseded by backend's fast build.)

**Prod verification (`192.168.4.217:3002`, Playwright):**
- **Mobile (390×844):** mobile bar + hamburger **visible**, desktop aside **hidden** (`display:none`). Tap ☰ → **drawer opens** (all 6 nav items with count chips — Active round/Shortlist/Chat watcher/Digest preview `3 NEW`/Round history/Setup — + `● ML OK` badge). Tap **"Digest preview"** → `/digest/97` (digest page), **drawer auto-closed**, bar persists. Tap **"✎ Regenerate with options…"** → **`GenerateModal` opens** and renders cleanly at 390px. ✅ Full path nav → digest → GenerateModal works on mobile.
- **Desktop (1280×900):** sidebar **visible at 232px (unchanged)**, hamburger **hidden**. ✅ Additive — desktop untouched.
- **PWA check:** `manifest.webmanifest` served on prod **HTTP 200 · application/manifest+json**, `"display":"standalone"`, `"start_url":"/"`; `<link rel="manifest">` present. The PWA opens at `/` in standalone (no browser chrome) and the nav bar lives in the **app DOM** (not browser chrome) at phone width — i.e. the mobile screenshots (page-content only, no chrome) are exactly the installed-PWA view, so the affordance is reachable in the standalone PWA. ✅
- Screenshots captured during verification (home bar, drawer-open, GenerateModal-on-mobile, desktop-unchanged) — described above; temp files removed per repo hygiene. **`npm run check` → 0 errors** (31 pre-existing warnings, none in `+layout.svelte`); **0 console errors** on prod.

**Lane hygiene:** touched only `+layout.svelte`; did NOT touch the Dockerfiles / deploy flow or any digest data/scoring. **Sprint-19 complete:** chromium-base-image ✅ · bot-ui-on-base ✅ · bot-on-base ✅ · deploy-flow-docs ✅ (backend) · mobile-digest-menu ✅ (frontend).
