...
### 2026-05-19 — frontend — T13 rel-context wired against live T12 (modal + PATCH + refresh persistence)
- **Backend T12 cheat-sheet ingested.** Finalize payload: `relContext: { previous, proposed, updatedAt, leagueId }` or `null` plus `warnings: string[]`. GET `/api/leagues/:leagueId/rel-context` returns `{ context, previousContext, updatedAt, previousUpdatedAt, lastRoundId, leagueId }`. PATCH body `{ context: string }`. **Snapshot invariant** (single writer `upsertRelContext`): every write snapshots current `(text, updated_at)` into `(previous_text, previous_updated_at)` before writing the new value. The diff baseline is whatever is in `previous_text` at read time.
- **Files changed:**
  - `ui/src/routes/digest/[roundId]/+page.server.ts`:
    - `CurrentRoundMeta` gains `league_id` (added to the `rounds JOIN seasons` query).
    - New exported type `RelContextSnapshot`. Helper `fetchRelContext(fetch, leagueId)` hits `GET /api/leagues/:leagueId/rel-context` via SvelteKit's internal fetch, returns the snapshot or `null` on any failure (network, !ok, non-string `context`). Failure is silent — the digest page still loads even if rel-context is unreachable.
    - Returns `relContext` alongside every stage payload (`prepare` / `refine` / `finalize`). Refresh-persisted.
  - `ui/src/routes/digest/[roundId]/+page.svelte`:
    - Removed the old client-only `relContextDiff: RelContextPayload | null` state. Replaced with a **two-source `$derived` `relDiff`**:
      1. Primary: `relContextFromFinalize` $state — populated by the latest finalize POST response. Surfaces the just-generated diff immediately, before `invalidateAll()` roundtrips.
      2. Fallback: `data.relContext` from the server load — surfaces refresh-persisted diff using `previousContext` (left col) + `context` (right col) when they differ.
    - Footer link gate: `{#if relDiff}` — automatically hides when context == previousContext (post-acceptance no-diff state) AND when `previousContext` is null (pre-first-finalize league).
    - `finalizeAndDownload()` now reads `body.relContext: { leagueId, previous, proposed }` (plus a non-fatal `body.warnings[]` surfaced via the existing toast). Pre-T12 fallback paths still degrade cleanly.
  - `ui/src/lib/digest/RelContextDiffModal.svelte`:
    - Two new props: `leagueId: number`, `onSaved?: (newContext: string) => void`, `onPatchError?: (message: string) => void`.
    - New `patching` $state gates Close/Esc/scrim and disables both action buttons during the PATCH (no half-state if user mashes buttons).
    - **Two action buttons in the footer:** `↺ Revert to previous` (secondary) + `✓ Accept proposed` (primary). Each calls `PATCH /api/leagues/:leagueId/rel-context` with `{ context: <chosen value> }` and goes through a single `confirm()` (the destination value is real persisted state — confirm before clobbering). On 200 → `onSaved(newContext)` → page invalidates → footer re-evaluates. On error → toast via `onPatchError`.
    - **Semantics in light of backend's snapshot invariant** (worth being explicit about for future readers):
      - `Accept proposed` posts current `context` back at the endpoint. Backend snapshots `text → previous_text`, then writes the same value to `text`. Net effect: `previousContext` is overwritten to match `context`, so the diff link **disappears** on refresh. This is the "I've reviewed the diff and accepted it, dismiss the indicator" UX.
      - `Revert to previous` posts `previousContext`. Backend snapshots current to `previous_text` and writes previous to `text`. Net: `context` and `previousContext` swap. The footer stays visible (now with the values swapped, in case the user wants to revert the revert).
- **`npm run check`:** unchanged baseline — 1 ERROR (pre-existing `vite.config.ts`), 31 WARNINGS, 14 FILES. Zero new diagnostics.
- **Deploy:** `docker compose build --no-cache bot-ui && docker compose up -d bot-ui`. Container Up on `0.0.0.0:3002`.
- **End-to-end smoke (against league 2 / Fam-Jam — the league round 14 belongs to; existing rel-context state from backend's T12 smoke):**
  1. **Refresh-persistence:** `GET /digest/14` → 200, HTML contains `dg-relctx-footer` + `view diff →`. League 2's `previousContext = "OPERATOR OVERRIDE …" (75 chars)` ≠ `context = "## People …" (2703 chars)`, so the diff footer surfaces *without* a fresh finalize click. ✓
  2. **Accept proposed (PATCH with current `context` echoed back):** `PATCH /api/leagues/2/rel-context` body `{"context":<current>}` → 200. New state: `context = 2703 chars` (same), `previousContext = 2703 chars` (was 75 — overwritten via snapshot). Reload `/digest/14` → no `dg-relctx-footer`, no `view diff` (footer correctly hidden because `context === previousContext`). ✓ matches the documented Accept UX.
  3. **Bring diff back with a marker PATCH:** `PATCH …` body `{"context":"REVERT TEST MARKER …"}` → 200. New state: `context = 51 chars` (marker), `previousContext = 2703 chars` (rotated from the prior `context`). Reload `/digest/14` → footer + `view diff` present. ✓
  4. **Revert to previous (PATCH with `previousContext` echoed back):** `PATCH …` body `{"context":<previousContext>}` → 200. New state: `context = 2703 chars` (rotated back), `previousContext = 51 chars` (marker rotated to the previous slot). Reload `/digest/14` → footer + `view diff` still present (now `context != previous` again). ✓ confirms snapshot-rotation semantics.
  5. **Cleanup:** PATCH with current → both slots converge to the canonical 2703-char LLM doc. Footer hidden. League state restored to a no-diff baseline so the smoke doesn't pollute the editorial corpus.
