...
- [ ] {agent: frontend, id: route-scaffold} Task 5: Route scaffold — `/digest/[roundId]` page with pipeline strip (static states)
...
- [ ] {agent: frontend, id: section-components} Task 6: Section components — all 6 variant-C section types rendered with fixture data
...
- [ ] {agent: backend, id: llm-service, depends: schema,api-scaffold} Task 7: LLM service — generate draft + per-section regen via OpenRouter
...
- [ ] {agent: backend, id: whole-regen, depends: llm-service} Task 8: Whole-draft regenerate — parallel regen skipping locked sections
