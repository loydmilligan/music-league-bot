...
- [x] {agent: frontend, id: season-page, depends: layout-shell, chip-badge-component} Re-skin the season detail page (`/league/[league]/season/[n]`): rounds list as a card grid matching prototype A's card shape, with deadline chip + member count + dot status per round.
...
- [x] {agent: frontend, id: wordmark-fix, depends: layout-shell} Replace the flat italic `m/l` span in `ui/src/routes/+layout.svelte` (currently `<span class="font-display italic text-accent text-2xl">m/l</span>` around line 63) with the canonical 3D-extruded mark recipe from `data/Mash Co. Design System-handoff/mash-co-design-system/project/music-league/brand/m-l-mark.html`. The recipe: Bricolage Grotesque **weight 800** italic, `letter-spacing: -0.06em`, `color: var(--color-accent)` (#ff5b2e), `-webkit-text-stroke: 2.5px var(--color-accent-deep)` (#8a2d15) with `paint-order: stroke fill`, a 7-step `text-shadow` extrusion stack in `--color-accent-deep` plus a final `rgba(0,0,0,0.45)` drop shadow at `extrude × 1.5` blur, and `transform: translateY(-4%)` for optical centering. Scale the extrude depth to fit sidebar size — at ~24–28px font-size that's ~2–3px per the spec's `extrudeFor(px) = max(2, round(px * 0.07))`. Add the styles as a scoped Svelte `<style>` block or inline `style=` attribute (not pure Tailwind utilities — `-webkit-text-stroke`, `paint-order`, and the multi-stop `text-shadow` stack aren't first-class Tailwind utilities, and a custom plugin is overkill for one usage).
  - **Acceptance:** Side-by-side screenshot of `/` left rail wordmark vs `data/Mash Co. Design System-handoff/.../brand/m-l-on-bone-512.png` (scaled appropriately) shows the 3D extruded edge in `--color-accent-deep`, the drop shadow, and the tight letter-spacing; plain italic glyph rendering is gone. Hover state still resolves (existing `group-hover:text-accent-strong` is fine — note that swapping the fill color requires also swapping `-webkit-text-stroke` if you want strict consistency on hover, but a fill-only hover is acceptable). The mark stays crisp at 1x and 2x DPR.

- [x] {agent: infra, id: favicons} Wire the brand favicon set from `data/Mash Co. Design System-handoff/mash-co-design-system/project/music-league/brand/` into the SvelteKit app. Copy (don't symlink — the Docker image needs the bytes) `m-l-favicon-32x32.png`, `m-l-favicon-64x64.png`, `m-l-favicon-180x180.png`, `m-l-favicon-256x256.png`, `m-l-favicon-512x512.png`, `m-l-apple-touch-180.png`, `m-l-maskable-512.png`, and `manifest.webmanifest` into `ui/static/` (SvelteKit serves `static/` at root). Update `ui/src/app.html` `<head>` with `<link rel="icon" type="image/png" sizes="32x32" href="/m-l-favicon-32x32.png">` (and the other size variants), `<link rel="apple-touch-icon" sizes="180x180" href="/m-l-apple-touch-180.png">`, `<link rel="manifest" href="/manifest.webmanifest">`. Audit the manifest's `icons[].src` paths and edit them to point at the `static/`-served paths (`/m-l-favicon-*.png` etc.) if the source manifest uses relative paths.
  - **Acceptance:** Visit `/` — browser tab shows the orange `m/l` mark as favicon; `curl http://localhost:5174/m-l-favicon-32x32.png` returns the PNG bytes (HTTP 200, `Content-Type: image/png`); `curl http://localhost:5174/manifest.webmanifest` returns valid manifest JSON with reachable icon paths; the docker container also serves the favicons (rebuild image and curl `http://localhost:3002/m-l-favicon-32x32.png`). No svelte-check regressions; no infra-side regressions in the existing `Dockerfile.ui` / `docker-compose.yml`.
...
### 2026-05-14 — frontend — round-page landed
- `ui/src/routes/league/[league]/season/[n]/round/[roundId]/+page.svelte` rewritten and `ui/src/lib/components/ResearchList.svelte` reskinned to match the prototype design system. **Sprint-2 is now 11/11 — final task complete.**
- **Page header:** breadcrumb `music-league-bot · {slug} · season {n} · round {id}` (`text-fg-faint font-mono text-xs tracking-widest uppercase`) with `music-league-bot` and `season {n}` as accent-hover links. H1 `{round name}` (`text-4xl font-bold text-fg`), `text-fg-muted` description below, then a mono `text-fg-dim` stats line: `N submissions · N chat mentions · N in research · winner: {topSong}` (winner appended only when round is archived). Right-aligned `<DeadlineChip>` for submissions/voting deadlines or `<StatusChip tone="muted">ARCHIVED</StatusChip>` for past rounds.
- **Tab strip:** `border-b border-border-muted` baseline; each tab is `font-mono text-sm uppercase tracking-wider py-3 -mb-px border-b-2`. Active tab gets `border-accent` + `text-accent` + `font-bold`; inactive `border-transparent text-fg-muted hover:text-fg`. Count badges `[N]` rendered in `font-mono text-[10px] text-fg-faint` next to each label.
- **ML tab:** song rows as `pl-4 pr-4 py-3 bg-surface border-l-2 border-border-muted hover:border-accent` strips. Each row: mono rank `#N`, bold title + muted `artists · submitter` line, 5-dot vote indicator (filled count = `round(points / maxPoints * 5)`, minimum 1 if positive points), `N pts` mono, and a Spotify/YT Music deep-link in the appropriate brand color. The Spotify/YT Music mode toggle restyled as a pill segmented control in the prototype palette (green health for Spotify, accent orange for YT Music). "Open in Spotify ↗" playlist link uses `text-health`.
- **Chat tab:** mirror styling — same `bg-surface border-l-2` strip with bold title + muted `artist · submitter`, mono health-tone source platform tag, mono faint date.
- **Research tab — `ResearchList.svelte` overhaul:**
  - **Section headers** now `font-mono text-xs tracking-widest uppercase text-fg-faint` ("Search Spotify", "Candidates [N]") instead of slate sans labels.
  - **Search input** uses `bg-bg border-border-muted focus:border-accent`; submit button is the accent-orange CTA in `font-mono tracking-widest uppercase`.
  - **Search result rows** use `bg-bg border-border-muted` strips with `font-mono text-[11px]` artist/album sub-line and an accent + Add chip.
  - **Candidate cards** use `bg-bg-elevated border-border-muted rounded-xl p-4` (page-level surface stays `bg-surface`, so candidate cards visually inset). Score readout is `font-display text-3xl` with a green/amber/dim tone based on the value (`>=4`/`>=3`/else). "Submitted by me" / "Submitted by other" badges restyled as accent / muted chips matching the rest of the design system.
  - **Rating UI** is now an orange dot row (`w-3 h-3 rounded-full bg-accent` for filled, `border-border` outline for empty) — matches prototype C's dot scoring. Clicking a dot sets that rating; clicking the same value again clears it (existing behavior preserved). Inline `clear` text affordance kept for explicit reset.
  - **Notes textarea** uses `bg-bg border-border-muted focus:border-accent`; same in/out state as before.
  - **Footer chips** — Save for future + Spotify/YT Music links in mono with token-correct hover states; Remove button moves to warn tone on hover.
- **Source data:** existing round `+page.server.ts` loader (untouched). Loader already returns ml_submissions, chat mentions, research_songs, and settings, so this was purely visual.
- **Verification:** `npm run dev` (port 5175 due to a stale 5174 listener) → `curl /league/second-best/season/1/round/97` HTTP 200 (55kb). Screenshots:
  - `docs/screenshots/2026-05-14-sprint2-round-page-ml.png` — New Shit round, ARCHIVED chip in header, ML tab active with 12 song rows showing dot-scaled votes (`Ceiling Granny` winner with 5 filled dots / 6 pts; `One Of Those Days` with 1 dot / -1 pts on the tail; mid-rank rows showing 3-dot fills).
...
- **Card grid:** rounds rendered via a `{#snippet roundCard()}` partial into two sections — `Active rounds` (orange-bordered card, `border-l-4 border-accent` + `bg-surface`, with an accent `N OPEN` StatusChip header) and `Archived rounds` (plain `bg-surface` card). Each round tile: `bg-surface border border-border-muted hover:border-accent-deep rounded-xl p-5`, contains a `<DeadlineChip>` (when there's a future submission or voting deadline) or `<StatusChip tone="muted">ARCHIVED</StatusChip>`, bold round name, optional description (line-clamped), `r-{id} · N submission(s)` mono dim line, and `{n} in research` accent line when research exists. Whole tile links to the round page.
- **Sort:** `isActive(r) = phaseFor(r) !== null` (i.e. a future submission or voting deadline). Active first, sorted by deadline asc; archived after, sorted by id desc.
...
- **`ui/static/manifest.webmanifest`** authored fresh from the brand-source template. Two fixes vs the source: (a) `icons[].src` paths changed from the brand-folder-relative `music-league/brand/m-l-favicon-*.png` to root-relative `/m-l-favicon-*.png` since SvelteKit serves `static/` at the root; (b) `start_url` changed from `/Music%20League%20Bot.html` (a brand-handoff demo target that doesn't exist in this app) to `/`. Kept `background_color` and `theme_color` at `#07090c` (matches design-tokens `--color-bg`).
- **`ui/src/app.html` `<head>`** gained, before the existing font preconnects: `<meta name="theme-color" content="#07090c" />`, four `<link rel="icon">` entries for 32/64/256/512, one `<link rel="apple-touch-icon" sizes="180x180">`, and `<link rel="manifest" href="/manifest.webmanifest" />`. Skipped 180x180 as a generic icon since it's already wired as the apple-touch-icon. No `favicon.svg` was in `ui/static/` to begin with, so nothing to remove or reorder.
- **Verification:**
  - `DATA_DIR=…/data npm run dev` (port 5174) → `curl -o /dev/null -w "%{http_code} %{content_type}"` for each asset:
    - `/m-l-favicon-32x32.png` → `200 image/png`
    - `/m-l-favicon-180x180.png` → `200 image/png`
    - `/m-l-apple-touch-180.png` → `200 image/png`
    - `/m-l-maskable-512.png` → `200 image/png`
    - `/manifest.webmanifest` → `200 application/manifest+json`
  - `curl /manifest.webmanifest | python3 -m json.tool` returns valid JSON with all 7 icon entries.
