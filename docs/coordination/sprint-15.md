...
- [x] {agent: backend, id: chat-restructure} **Restructure the chat section's generation output** from one prose blob to a structured shape: `{ summary: string, moments: [{ label: string, detail: string }] }` — a short overall summary plus a list of discrete chat moments, each with a brief label and a fuller description. Keep the "found the funny content + slightly-funny tone" quality that's working; only change the structure. Emit it as the chat section's `content_json`.
...
- [x] {agent: frontend, id: chat-wire, depends: chat-render} **Mount viz's `ChatMoments` into the chat-section rendering.** Gap found in UAT: `ChatMoments.svelte` (viz, commit 53b8ca8) is built but mounted nowhere — `DigestSection.svelte` renders sections via hardcoded `kind ===` branches with no `chat` case, so the restructured chat `content_json` (`{ title, summary, moments:[{label,detail}] }` from backend's chat-restructure) does NOT render through ChatMoments. Wire it: render the chat section via `ChatMoments` (analogous to how `standings-wire` registered `StandingsChart`) — add the chat case to `DigestSection`/the variant path, passing the chat `content_json` through. Web = expandable, PDF/static = anchor-linked (ChatMoments already handles both variants).
  - **Acceptance:** on prod, a digest's chat section renders via `ChatMoments` — summary on top + the moments (expandable on web, anchor-linked in the PDF export), not raw/mis-rendered structured content. `npm run check` passes; deployed; visual check on prod.

- [x] {agent: backend, id: cost-capture} **Capture the OpenRouter LLM cost per digest.** For each digest's generation (draft + any regens/section gens), capture the OpenRouter cost (from the API response usage / the OpenRouter generation cost), accumulate the total per digest, and persist it (e.g. a `llm_cost_usd` column on `digest_drafts` or a per-digest cost record). Expose the accumulated cost in the digest data the page loads.
...
### 2026-06-03 — backend — Wave 1 ALL 3 backend tasks done + deployed + prod-smoked

- **chat-restructure** — the chat section's LLM output is now structured. **Shape for viz** (the chat section's `content_json`):
  `{ "title": string, "summary": string, "moments": [{ "label": string, "detail": string }] }`
  (`title` retained for parity with other sections; render `summary` + `moments`). 3-6 moments; dry/funny voice preserved; self-suppress unchanged (no chat data → section omitted upstream by `activeKindsForDraft`).
  - **Prod smoke (r95 + pastedChat):** chat `content_json` keys `[moments, summary, title]`, summary populated, 3 moments each with `label`+`detail` (e.g. `"Unanimous Verdict: Unreal"`).
- **podium-thumbnails** — album art is **not** in the ML export (r104 coverage was 0/9), so it's resolved **from Spotify** (existing `client_credentials` client; new `fetchTracks` batch ≤50) and cached on **`ml_submissions.album_art_url`**. `enrichPodiumArt` injects `coverUrl` into the podium section's `content_json.items` (title-match → vote-rank-position fallback, never overrides existing). Runs at gen + on podium regen; `ensureAlbumArt` runs best-effort before generation (Spotify failure can't block gen). Cached/older drafts are **backfilled on read** (additive `coverUrl` only — no prose change).
  - **Prod smoke:** fresh r95 gen → 3/3 podium items carry real `i.scdn.co` cover URLs; **r104's existing finalized draft backfilled to 3/3 covers without regenerating** → the visual podium now renders real covers on prod.
- **cost-capture** — `callOpenRouter` sends `usage:{include:true}` and returns `{ content, costUsd }` (reads `usage.cost`, USD). **Field for frontend:** `digest_drafts.llm_cost_usd` (REAL, accumulated) — surfaced on the draft row the page loads as **`draft.llm_cost_usd`**. Accumulates the draft gen + every whole-draft regen + every section regen (`addDraftCost`).
  - **Prod smoke (r95):** fresh draft recorded `llm_cost_usd = 0.042396`; a whole-draft regen accumulated it to `0.138969`. (Pre-existing drafts like r104 read `0` until next gen — expected.)
- **Verification:** `npm run check` 0 errors; full vitest **113 passing** (+3 new: chat shape, podium enrichment ×2). DB migrations (idempotent ALTERs) added for both new columns on existing DBs. Deployed serialized (no concurrent `up`): `build --no-cache bot-ui && up -d --force-recreate`; all smoked live on `192.168.4.217:3002`. r95 test draft cleaned up after.
- **Lane note:** backend only — LLM/cost/album-art data + `SECTION_KINDS`-adjacent generation. Did **not** touch the digest `.svelte` page wiring, the cost-display UI, or any visual component. **➡ viz:** `chat-render` is unblocked (shape above). **➡ frontend:** `cost-display` is unblocked (`draft.llm_cost_usd`).

### 2026-06-03 — frontend — chat-wire + cost-display DONE + podium tie hotfix; SPRINT-15 FINAL PASS ✅

**chat-wire (id: chat-wire) — DONE, prod-verified.** Root cause: `ChatMoments` was built but mounted nowhere; `DigestSection` had no `chat` case (the structured `{title,summary,moments[]}` fell through the generic textual path → empty). Fix (page wiring): `VISUAL_CAPABLE.chat = true` + registered `chat: ChatMoments` in `VISUAL_COMPONENTS`; chat sections default to the visual slot (`effectiveSectionVariant` maps chat `textual`→`visual`, since restructured chat has no textual form). ChatMoments self-detects export mode (`?export=1`) → web = expandable accordion (`chatm-acc`, 6 `chatm-item`/`chatm-trigger`/`chatm-label`), PDF = anchor-linked. Verified: chat renders 6 moments + summary on prod r-104.

**cost-display (id: cost-display) — DONE, prod-verified.** Shows `draft.llm_cost_usd` (`$0.0426` on r-104) at the top of the **finalized** view via a `.dg-cost-banner`. Excluded from the shared artifact two ways: it lives **outside `.dg-export`** (the PNG element-screenshot only captures `.dg-export`) **and** carries `data-export-hide="1"` (hidden in the full-page PDF render). Confirmed via in-container Puppeteer: banner present in DOM, `display:none` after the export-hide CSS, `closest('.dg-export')` = false.

**⚠ Cross-lane hotfix (AlbumPodium.svelte — viz's file; viz idle, page-breaking):** during the final pass, finalize failed with `Waiting for selector .dg-export failed`. In-container Puppeteer surfaced the real cause: `PAGEERROR https://svelte.dev/e/each_key_duplicate` — a **fatal hydration error that wiped the entire page** (broke export AND the browser web view). AlbumPodium keyed `{#each shelf/rest as s (s._rank)}` by rank, but **ranks tie** (r-104 podium = `[1,1,3]`, the Jeremy/You-Learn 15-pt tie) → duplicate key. Fixed by keying on a stable unique `_i` instead. Surgical, behavior-preserving. **viz: heads-up — your other rank-keyed eaches should use a unique id, not rank.**

**FINAL PASS (prod r-104, all four together):**
- ✅ season-standings chart (10 rows: season bars + this-round segments + rank/prev-rank + arrows + legend)
- ✅ album-art podium (9 real Spotify covers, medal cards — backend podium-thumbnails populated r-104)
- ✅ chat via ChatMoments (summary + 6 expandable moments; anchor-linked in PDF)
